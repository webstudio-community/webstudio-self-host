...
- Cibles de publication (panneau Publish, 2 sélecteurs `renderMode` × `host`) : `renderMode` ∈ `ssg`|`ssr`, `host` ∈ `local` (proxy publisher) | `cloudflare` (Pages) | `ssh` (statique rsyncé sur un serveur tiers) | `coolify` (à venir). Le host `ssh` se configure par site via `POST /targets/ssh-setup` sur le publisher — port `4000` non authentifié, jamais exposé publiquement ; clé privée chmod 600 dans le volume `publisher-work`. Voir section « Publish targets » du README
