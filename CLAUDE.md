...
- Cibles de publication (panneau Publish, 2 sélecteurs `renderMode` × `host`) : `renderMode` ∈ `ssg`|`ssr`, `host` ∈ `local` (proxy publisher) | `cloudflare` (Pages) | `ssh` (statique rsyncé sur un serveur tiers) | `coolify` (SSR sur un Coolify distant). `ssh` : config par site via `POST /targets/ssh-setup` sur le publisher (port `4000` non authentifié, jamais exposé ; clé chmod 600 dans `publisher-work`). `coolify` : `REGISTRY_URL`/`REGISTRY_USER`/`REGISTRY_TOKEN` sur le publisher (registre où pousser l'image SSR) + `docker.sock` monté (mount commenté) + webhook de déploiement Coolify saisi par site dans le panneau Publish. Voir section « Publish targets » du README
